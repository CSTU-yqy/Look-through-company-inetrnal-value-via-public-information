...
from tool.model_kernel_complement_globalTF import Model
...
"""
"""
When the number of cores of the computer is less than the number of processes I want to run, I will rewrite the program to the following form

"""
def predict(min_df,org_list,seed_list,vote_num = 7,model_alpha = 0.001):
    for org,seed in zip(org_list,seed_list):
    ...
    #all_data = pd.read_pickle("/Users/qianyuyang/Desktop/paper/data/important/sentence_target_cut_already.pkl")
        ...
        for vote in range(vote_num):
            ...
            for line_standard in range(20,0,-1):
                ...
                m = Model(org = org,line_standard = line_standard,min_df = min_df,model_alpha = model_alpha)
                k = m.result
                org = k["prediction"]
            ...
            """
            again_result = Model(org = TOOl().clean(min_df = min_df,org = grand_org),line_standard = best_key,min_df = min_df,model_alpha = model_alpha).result
